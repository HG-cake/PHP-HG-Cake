...
$sql='select a.`id`,a.`lm`,a.`title`,a.`img_sl`,a.`px`,a.`read_num`,a.`pass`,a.`tuijian`,a.`hot`,b.`title_lm` from `'.$tablepre.'pro_co` a left join `'.$tablepre.'pro_lm` b on a.`lm`=b.`id_lm` where `pass`<>"" '.$sq.' order by a.`px` desc,a.`id` desc';
$p=new page();
...
?>
    <tr class="tdbg">
        ...
        <td><?php echo '<b>[</b>'.$row['title_lm'].'<b>] </b><a href="edit.php?id='.$row['id'].'" >'.$row['title'].'</a> '.(($row['img_sl']!='')?'<a href="../../'.$row['img_sl'].'"  target=_blank><img src="../images/img.gif" border="0" onmouseover="popImage(this,\'../../'.$row['img_sl'].'\');" onmouseout="hideLayer();"/></a>':'')?></td>
