...
if($ac==''){
	if($img_sl==''){
		...
		echo'&nbsp;<img src="../images/img.gif" onmouseover="popImage(this,\'../../'.$img_sl.'\');" onmouseout="hideLayer();">　<a href="uploadd.php?frameid='.$frameid.'&kuang='.$kuang.'&img_sl='.$img_sl.'&ac=del" onclick="return confirm(\'确定要删除文件吗？\')">删除文件,重新上传</a>';
	}
}elseif($ac=='del'){
	if($img_sl!=''){
		delfile($img_sl);
		delfile(substr($img_sl,0,strrpos($img_sl,'/')+1).'d'.substr($img_sl,strrpos($img_sl,'/')+1));
		delfile(substr($img_sl,0,strrpos($img_sl,'/')+1).'z'.substr($img_sl,strrpos($img_sl,'/')+1));
		delfile(substr($img_sl,0,strrpos($img_sl,'/')+1).'x'.substr($img_sl,strrpos($img_sl,'/')+1));
	}
