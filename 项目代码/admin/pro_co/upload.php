...
if (isset($_FILES['file_up'])){
	...
	if($row=$up->upLoad($file,$sava_path,$file_name,$allow_types,$max_size,$overfile)){
		...
		if($s_pic){
			...
			//$x_name=substr($row['name'],0,strrpos($row['name'],'/')+1).'x'.substr($row['name'],strrpos($row['name'],'/')+2);
			$s_name=substr($row['name'],0,strrpos($row['name'],'/')+1).substr($row['name'],strrpos($row['name'],'/')+2);
			$up->makesmall($row['name'],false,260,273,$z_name);
			//$up->makesmall($row['name'],true,182,182,$x_name);
			$up->makesmall($row['name'],false,221,232,$s_name);
		}
		if ($frameid!=''&&$kuang!=''){
