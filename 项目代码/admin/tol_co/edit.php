...
if (!$row=$db->getrow($result)){
	...
	$sql='select * from `'.$tablepre.'tol_lm` where `id_lm`='.$row["lm"].'';
	$res=$db->query($sql);
	if ($rs=$db->getrow($res)){
		$id_lm=$rs['id_lm'];
		$add_xx=$rs['add_xx'];
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="border">
    ...
    <select name="lm" id="lm">
      <option value="n" selected="selected">请选择栏目</option>
		...
		function addlm($fid,$i){
			global $db,$tablepre,$id_lm,$add_xx;
			if ($i==''){
				$i='• ';
			}elseif ($i=='• '){
				$i=' 　|—';
			...
			while($rsk=$db->getrow($rek)){
				...
				}elseif($id_lm==$rsk['id_lm']&&$add_xx=='no'){
					echo'<option value="'.$rsk["id_lm"].'">'.$i.$rsk["title_lm"].'</option>'."\n";
				}else{
  ...
  <tr class="tdbg">
    ...
    <td><INPUT name="title" type="text" id="title" size="30" maxlength="150" value="<?php echo $row['title']?>" <?php if($add_xx=='no'){echo'disabled="disabled"';}?>> <?php if ($add_xx=='no'){ echo'<input name="title" id="title" type="hidden" value="'.$row["title"].'"/>';}?><span class="red">*</span></td>
  </tr>
  <tr class="tdbg">
